                ...
                <v-menu location="bottom end" :close-on-content-click="true">
                    ...
                    <v-card class="orders-filter-tray orders-actions-tray rounded-xl border-thin shadow-xl" :class="theme.global.current.value.dark ? 'orders-filter-tray-dark text-white' : 'orders-filter-tray-light'" width="350">
                        ...
                        <div class="tray-section px-3 pb-3">
                            <button class="tray-action tray-action--settings" type="button" @click="openGridSettings">
                                <span class="tray-action-icon text-indigo"><v-icon size="18">mdi-table-cog</v-icon></span>
                                <span>
                                    <strong>Configurar colunas</strong>
                                    <small>Escolha quais colunas aparecem na grid.</small>
                                </span>
                            </button>
                        </div>
        ...
        <v-card class="flex-grow-1 d-flex flex-column rounded-0 border-thin overflow-hidden" :class="theme.global.current.value.dark ? 'glass-card border-white-05' : 'bg-white shadow-sm'" :elevation="0">
          ...
          <div v-if="!mobile" class="grid-scroll flex-grow-1 overflow-y-auto custom-scroll">
            <div class="grid-header sticky-head" :class="theme.global.current.value.dark ? 'grid-surface-dark' : 'grid-surface-light'" :style="{ gridTemplateColumns: computedGridTemplate }">
               <div v-if="isColumnVisible('order')" class="cell center header-text">#</div>
               <div v-if="isColumnVisible('emission')" class="cell center header-text">Emissão</div>
               <div v-if="isColumnVisible('client')" class="cell header-text">Cliente / Responsável</div>
               <div v-if="isColumnVisible('company')" class="cell center header-text">Empresa</div>
               <div v-if="isColumnVisible('seller')" class="cell center header-text">Vendedor</div>
               <div v-if="isColumnVisible('qty')" class="cell center header-text">Qtd</div>
               <div v-if="isColumnVisible('delivery')" class="cell center header-text">Entrega</div>
               <div v-if="isColumnVisible('production')" class="cell center header-text">Status Produção</div>
               <div v-if="isColumnVisible('finance')" class="cell center header-text">{{ activeOrderMode === 'sales' ? 'Recebimento' : 'Financeiro' }}</div>
               <div v-if="isColumnVisible('fiscal')" class="cell center header-text">Fiscal</div>
               <div v-if="isColumnVisible('signal')" class="cell center header-text">Sinal</div>
               <div v-if="isColumnVisible('pending')" class="cell center header-text text-error">Pendências</div>
               <div v-if="isColumnVisible('total')" class="cell center header-text">Total</div>
               <div v-if="isColumnVisible('actions')" class="cell center header-text">Ações</div>
            </div>
            ...
            <div v-else v-for="(item, index) in paginatedOrders" :key="item.id" class="grid-row" :class="theme.global.current.value.dark ? 'grid-row-dark' : 'grid-row-light'" :style="{ gridTemplateColumns: computedGridTemplate }">
               <div v-if="isColumnVisible('order')" class="cell center">
               ...
               <div v-if="isColumnVisible('emission')" class="cell center">
                   <span class="list-text-10 font-mono opacity-80">{{ formatDate(item.created_at) }}</span>
               </div>
               <div v-if="isColumnVisible('client')" class="cell flex-grow-1">
              ...
              <div v-if="isColumnVisible('seller')" class="cell center">
                   <span class="text-truncate list-text-10 opacity-70">{{ item.creator?.full_name?.split(' ')[0] || 'Sistema' }}</span>
               </div>

                <div v-if="isColumnVisible('qty')" class="cell center"><span class="list-text-11 font-mono font-weight-bold">{{ formatNumber(item.quantity_meters || 0) }}m</span></div>

               <div v-if="isColumnVisible('delivery')" class="cell center">
               ...
               <div v-if="isColumnVisible('production')" class="cell center">
                   <span class="solid-chip chip-3d" :class="`chip-${getStatusColor(item.status)}`">{{ formatStatus(item.status) }}</span>
               </div>

              <div v-if="isColumnVisible('finance')" class="cell center">
               ...
               <div v-if="isColumnVisible('fiscal')" class="cell center">
               ...
               <div v-if="isColumnVisible('signal')" class="cell center">
               ...
               <div v-if="isColumnVisible('pending')" class="cell center">
               ...
               <div v-if="isColumnVisible('total')" class="cell right"><span class="list-text-11 font-mono font-weight-black">{{ formatCurrency(item.total_value) }}</span></div>

               <div v-if="isColumnVisible('actions')" class="cell center">
...
<div v-else class="flex-grow-1 overflow-y-auto bg-grey-lighten-4 pb-12 custom-scroll">
  ...
  <div v-else class="mobile-cards-wrap pa-4">
    ...
    <v-dialog v-model="gridSettings.showModal" max-width="520" scrim="black opacity-70">
        <v-card class="orders-grid-settings rounded-xl overflow-hidden" :class="theme.global.current.value.dark ? 'bg-grey-darken-4 text-white' : 'bg-white'">
            <div class="px-5 py-4 border-b d-flex align-center justify-space-between" :class="theme.global.current.value.dark ? 'bg-grey-darken-3' : 'bg-grey-lighten-5'">
                <div class="d-flex align-center">
                    <div class="grid-settings-icon mr-3"><v-icon size="20">mdi-table-cog</v-icon></div>
                    <div>
                        <div class="text-caption font-weight-black opacity-70 text-uppercase">Personalização</div>
                        <div class="text-h6 font-weight-black">Configurar colunas</div>
                    </div>
                </div>
                <v-btn icon variant="text" @click="gridSettings.showModal = false"><v-icon>mdi-close</v-icon></v-btn>
            </div>
            <div class="pa-5">
                <div class="text-subtitle-2 font-weight-bold mb-2" :class="theme.global.current.value.dark ? 'text-white' : 'text-grey-darken-2'">Colunas visíveis</div>
                <div class="text-caption opacity-70 mb-4">Desmarque as colunas que você não quer ver nesta tela. As preferências ficam salvas para seu usuário.</div>
                <v-list density="compact" class="orders-columns-list rounded-lg border-thin" :class="theme.global.current.value.dark ? 'bg-grey-darken-3 text-white' : 'bg-grey-lighten-5'">
                    <template v-for="col in gridSettings.columns" :key="col.key">
                        <v-list-item v-if="!col.globalOnly || companyStore.isGlobalView" class="orders-column-item rounded-lg">
                            <template v-slot:prepend>
                                <v-checkbox-btn v-model="col.visible" color="success" density="compact" hide-details :disabled="col.locked" />
                            </template>
                            <v-list-item-title class="font-weight-bold text-body-2">{{ col.label }}</v-list-item-title>
                            <v-list-item-subtitle class="text-caption opacity-70">{{ col.hint }}</v-list-item-subtitle>
                        </v-list-item>
                    </template>
                </v-list>
            </div>
            <div class="px-5 py-4 border-t d-flex justify-end gap-3" :class="theme.global.current.value.dark ? 'bg-grey-darken-3' : 'bg-grey-lighten-5'">
                <v-btn variant="outlined" class="btn-3d font-weight-bold" height="40" @click="resetGridSettings" :disabled="gridSettings.loading">Restaurar padrão</v-btn>
                <v-btn color="success" variant="flat" class="btn-3d px-6 font-weight-black" height="40" :loading="gridSettings.loading" @click="saveGridSettings">Salvar</v-btn>
            </div>
        </v-card>
    </v-dialog>
...
// MULTI-TENANT
const defaultOrderGridColumns = [
    { key: 'order', label: 'Pedido', visible: true, width: '62px', globalOnly: false, locked: true, hint: 'Número do pedido.' },
    { key: 'emission', label: 'Emissão', visible: true, width: '82px', globalOnly: false, locked: false, hint: 'Data em que o pedido foi criado.' },
    { key: 'client', label: 'Cliente / Responsável', visible: true, width: 'minmax(165px, 1.1fr)', globalOnly: false, locked: true, hint: 'Cliente ou responsável pelo pedido.' },
    { key: 'company', label: 'Empresa', visible: true, width: '88px', globalOnly: true, locked: false, hint: 'Tenant/empresa do pedido na visão Holding.' },
    { key: 'seller', label: 'Vendedor', visible: true, width: '92px', globalOnly: false, locked: false, hint: 'Responsável comercial.' },
    { key: 'qty', label: 'Qtd / Metragem', visible: true, width: '70px', globalOnly: false, locked: false, hint: 'Metragem total do pedido.' },
    { key: 'delivery', label: 'Entrega', visible: true, width: '110px', globalOnly: false, locked: false, hint: 'Previsão ou situação de entrega.' },
    { key: 'production', label: 'Produção', visible: true, width: '118px', globalOnly: false, locked: false, hint: 'Status operacional do pedido.' },
    { key: 'finance', label: 'Recebimento / Financeiro', visible: true, width: '106px', globalOnly: false, locked: false, hint: 'Situação financeira conforme o modo da tela.' },
    { key: 'fiscal', label: 'Fiscal', visible: true, width: '112px', globalOnly: false, locked: false, hint: 'Selo NFe/NFCe e disponibilidade de documento.' },
    { key: 'signal', label: 'Sinal', visible: true, width: '54px', globalOnly: false, locked: false, hint: 'Indica se houve sinal.' },
    { key: 'pending', label: 'Pendências', visible: true, width: '126px', globalOnly: false, locked: false, hint: 'Parcelas em aberto ou saldo restante.' },
    { key: 'total', label: 'Total', visible: true, width: '132px', globalOnly: false, locked: true, hint: 'Valor total do pedido.' },
    { key: 'actions', label: 'Ações', visible: true, width: '178px', globalOnly: false, locked: true, hint: 'Botões de ação do pedido.' }
];

const gridSettings = reactive({
    showModal: false,
    loading: false,
    columns: defaultOrderGridColumns.map(col => ({ ...col }))
});
...
const openGridSettings = () => {
    gridSettings.showModal = true;
};

const resetGridSettings = () => {
    gridSettings.columns.splice(0, gridSettings.columns.length, ...defaultOrderGridColumns.map(col => ({ ...col })));
};
...
const loadGridSettings = async () => {
    ...
    try {
        ...
        const prefs = data?.settings?.order_management_grid;
        if (!prefs?.columns) return;

        prefs.columns.forEach((savedCol: any) => {
            const target = gridSettings.columns.find(c => c.key === savedCol.key);
            if (target && !target.locked) target.visible = savedCol.visible;
        });
    } catch (e) {
        console.warn('Preferências da grid de pedidos ainda não salvas, usando padrão.');
    }
};
...
const saveGridSettings = async () => {
    ...
    try {
        ...
        currentSettings.order_management_grid = {
            columns: gridSettings.columns.map(c => ({ key: c.key, visible: c.visible }))
        };

        const { error } = await supabase
            .from('user_preferences')
            .upsert({ user_id: userStore.user.id, settings: currentSettings }, { onConflict: 'user_id' });
        ...
        console.error(e);
        appStore.showSnackbar('Não foi possível salvar as configurações da grid.', 'error');
    } finally {
...
.grid-settings-icon {
  ...
  align-items: center;
  justify-content: center;
  box-shadow: inset 0 1px 0 rgba(255,255,255,.65), 0 8px 18px rgba(15,23,42,.08);
}
...
.orders-filter-tray-dark .orders-column-item {
