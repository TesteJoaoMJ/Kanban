<template>
  ...
  <div v-else>
    <v-row>
      <v-col v-for="(value, key) in overview.kpis" :key="key" cols="12" sm="6" md="4" lg="2">
...
const kpiMeta = {
  ...
  // CAMPOS QUE FALTAVAM E CAUSARAM O CRASH:
  total_meters: { icon: 'mdi-ruler-square', color: 'deep-purple' },
  avg_meters_per_order: { icon: 'mdi-chart-line-variant', color: 'teal' },
};
...
const formatKpiTitle = (key) => ({
  ...
  // TÍTULOS QUE FALTAVAM:
  total_meters: 'Metragem Total',
  avg_meters_per_order: 'Média (m) / Pedido'
})[key] || key;

const formatKpiValue = (key, value) => {
  if (key === 'total_revenue' || key === 'avg_ticket') return formatCurrency(value);
  // FORMATAÇÃO QUE FALTAVA:
  if (key === 'total_meters' || key === 'avg_meters_per_order') return (value || 0).toFixed(2) + ' m';
