...
export const useDashboardStore = defineStore('dashboard', {
  ...
  getters: {
            ...
            if (order.is_launch && order.order_items) {
                order.order_items.forEach(item => {
                    if (item.has_insufficient_stock && !countedItems.has(item.id)) {
                        count++;
                        // CORREÇÃO: Number()
                        totalMeters += Number(item.quantity_meters) || 0;
    ...
    monthlyProduction(state) {
      ...
      // CORREÇÃO: Voltando a usar o cabeçalho do pedido (igual ao antigo) e convertendo string para número
      const totalMeters = filteredOrders.reduce((sum, order) => {
          return sum + (Number(order.quantity_meters) || 0);
      }, 0);
    ...
    totalMetersPendingApproval(state): number {
        return state.orders
            ...
            // CORREÇÃO: Number()
            .reduce((sum, item) => sum + (Number(item.quantity_meters) || 0), 0);
    },
    ...
    totalMetersInProduction(state): number {
        ...
        return state.orders
            ...
            // CORREÇÃO: Usando cabeçalho com Number()
            .reduce((sum, order) => sum + (Number(order.quantity_meters) || 0), 0);
    },
    ...
    salesBySeller(state): { seller: string, totalMeters: number }[] {
        ...
        this.filteredOrdersForCharts.forEach(order => {
            ...
            // CORREÇÃO: Usando cabeçalho com Number()
            const orderMeters = Number(order.quantity_meters) || 0;
            const currentMeters = salesMap.get(sellerName) || 0;
            salesMap.set(sellerName, currentMeters + orderMeters);
        });
        ...
        this.filteredOrdersForCharts.forEach(order => {
            if (order.is_launch) {
                order.order_items.forEach(item => {
                    ...
                    // CORREÇÃO: Number() no item
                    fabricMap.set(fabricName, currentMeters + (Number(item.quantity_meters) || 0));
                });
            } else if(order.details?.fabric_type) {
                const fabricName = normalizeFabricName(order.details.fabric_type);
                const currentMeters = fabricMap.get(fabricName) || 0;
                // CORREÇÃO: Number() no header
                fabricMap.set(fabricName, currentMeters + (Number(order.quantity_meters)||0));
            }
        });
        ...
        this.filteredOrdersForCharts.forEach(order => {
            ...
            if (isValid(orderDate)) {
                ...
                if (monthMap.has(monthKey)) {
                    ...
                    monthMap.set(monthKey, currentMeters + orderMeters);
                }
            }
        });
        return {
      ...
      myOrders.forEach(order => {
        ...
        if (isWithinInterval(orderDate, { start: thirtyDaysAgo, end: today })) {
          ...
          if (salesByDay.has(dayKey)) {
            // CORREÇÃO: Usando cabeçalho com Number()
            const orderMeters = Number(order.quantity_meters) || 0;
            salesByDay.set(dayKey, (salesByDay.get(dayKey) || 0) + orderMeters);
          }
        }
      });
